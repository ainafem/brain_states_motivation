...
def main():
    with pymp.Parallel(16) as p:
        for i in p.range(25, 36):
            ...
            p.print("LOADED", i)
            results, filterr = block_results(data, i, p)
            df = pd.DataFrame({'results' : results, 'filter': filterr})
            df.to_csv("Results/subject_"+str(i)+"_results_raw.csv", index=False)
