...
def block_results(data, num, p):
    ...
    for filt in filterrs:
        ...
        df.to_csv("Results/electrodesRFE/subject_"+str(num)+"_results_electrodes_RFE_"+filt+".csv", index=False)
...
def main():
    with pymp.Parallel(16) as p:
        for i in p.range(25, 36):
            ...
            df = pd.DataFrame({'results' : results, 'filter': filterr, 'size': size})
            df.to_csv("Results/subject_"+str(i)+"_results_electrodes_RFE.csv", index=False)
