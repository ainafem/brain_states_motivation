...
def preprocess_data(data, filt):
    ...
    invalid_ch = np.any(np.isnan(b), axis=1)
    valid_ch = np.logical_not(invalid_ch)
    b = b[valid_ch, :]
    labels = labels[valid_ch]
    
    invalid_ch = np.sum(b, axis=1) == 0
    valid_ch = np.logical_not(invalid_ch)
    b = b[valid_ch, :]
    labels = labels[valid_ch]
...
def block_results(data, num, p):
    ...
    for filt in filterrs:
        ...
        for s in range(2, 11):
            ...
            estimator=LogisticRegression(C=10, penalty='l2', multi_class='multinomial', solver='lbfgs', max_iter=500)
            selector = RFE(estimator, n_features_to_select=s, step=1)
            selector = selector.fit(X, labels)
            X_r = X[:, selector.support_]
        ...
        df.to_csv("Results/RFE/subject_"+str(num)+"_results_RFE_"+filt+".csv", index=False)
...
def main():
    with pymp.Parallel(16) as p:
        for i in p.range(25, 36):
            ...
            data = load_file['ic_data3']
            p.print("LOADED", i)
            results, filterr, size = block_results(data, i, p)
            df = pd.DataFrame({'results' : results, 'filter': filterr, 'size': size})
            df.to_csv("Results/subject_"+str(i)+"_results_RFE.csv", index=False)
